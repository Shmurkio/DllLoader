...
{
    if (!TestCrt()) return 1;
    if (!TestNewDelete()) return 2;
    if (!TestStringVectorAlgorithm()) return 3;
    if (!TestBasicStlTypes()) return 4;
    if (!TestSharedWeakPtr()) return 5;
    if (!TestVariantAny()) return 6;
    if (!TestDequeList()) return 7;
    if (!TestAssociativeContainers()) return 8;
